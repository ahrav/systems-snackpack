...
which requires the expected source commit, archive digest, hostname, toolchain
identity, and source-manifest digest:
...
python3 -I -B experiment/validate_receipts.py --root XXL_ROOT \
  ...
  --expected-source-manifest-sha256 b6ba0132715ce3e3ea81f373664c50bc34c7babe2ec4c616601137654af659d1
...
source, produced on another host, built with another compiler, or recording an
incomplete source manifest. Both bundles record the same manifest digest, so one
expected value serves both. The `xxl`
