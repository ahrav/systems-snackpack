...
which requires the expected source commit, archive digest, and hostname:
...
The expected identity is supplied by the verifier rather than read back from the
bundle, so the validator cannot report a pass for a bundle built from different
source or produced on another host. The `xxl` hostname above is the runtime
resolution recorded in `xxl-resolution.txt`; the Arm hostname is the fixed
authorized label.
