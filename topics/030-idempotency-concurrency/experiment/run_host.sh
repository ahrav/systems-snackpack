...
finalize() {
	...
	if [[ -n $cargo_home && -f $cargo_home/.topic30-cargo-home ]]; then
		rm -rf "$cargo_home"
	fi
	if [[ -n $build_root && -f $build_root/.topic30-build-root ]]; then
		rm -rf "$build_root"
	fi
	if [[ -d $output ]]; then
...
if [[ ! -d "$repository/topics/030-idempotency-concurrency" ]]; then
	echo "Topic 30 source is absent from repository: $repository" >&2
	exit 2
fi
if [[ -e "$output" ]]; then
...
build_root=$(mktemp -d "${TMPDIR:-/tmp}/topic30-build-root.XXXXXXXX")
touch "$build_root/.topic30-build-root"
tar -xzf "$output/source.tar.gz" -C "$build_root"
topic="$build_root/topics/030-idempotency-concurrency"
...
cargo_home=$(mktemp -d "${TMPDIR:-/tmp}/topic30-cargo-home.XXXXXXXX")
touch "$cargo_home/.topic30-cargo-home"
...
run_gate cargo-test-package-generic cargo test --locked \
	--package idempotency-concurrency
run_gate cargo-build-package-generic cargo build --locked --release \
	--package idempotency-concurrency --bin idempotency-probe

generic_binary="$build_root/target/release/idempotency-probe"
cp "$generic_binary" "$output/idempotency-probe.generic"
sha256sum "$output/idempotency-probe.generic" >"$output/binary.generic.sha256"
...
python3 -I "$topic/experiment/validate_receipts.py" \
	"$output/experiment-generic" "$output/idempotency-probe.generic" \
	>"$output/validation.generic.log" 2>&1
generic_recorded_sha256=$(awk 'NR == 1 { print $1 }' \
	"$output/experiment-generic/binary.sha256")
generic_actual_sha256=$(sha256sum "$output/idempotency-probe.generic" |
	awk '{ print $1 }')
...
binary="$build_root/target/release/idempotency-probe"
cp "$binary" "$output/idempotency-probe.native"
sha256sum "$output/idempotency-probe.native" >"$output/binary.native.sha256"
nm -n "$output/idempotency-probe.native" >"$output/binary.symbols.txt"
symbols=(
	topic30_begin_decision
	topic30_finish_allowed
)
for symbol in "${symbols[@]}"; do
	rg -q "[[:space:]][Tt][[:space:]]${symbol}$" \
		"$output/binary.symbols.txt"
	objdump -d --no-show-raw-insn --disassemble="$symbol" \
		"$output/idempotency-probe.native"
...
python3 -I "$topic/experiment/validate_receipts.py" \
	"$output/experiment-native" "$output/idempotency-probe.native" \
	>"$output/validation.native.log" 2>&1
native_recorded_sha256=$(awk 'NR == 1 { print $1 }' \
	"$output/experiment-native/binary.sha256")
native_actual_sha256=$(sha256sum "$output/idempotency-probe.native" |
	awk '{ print $1 }')
