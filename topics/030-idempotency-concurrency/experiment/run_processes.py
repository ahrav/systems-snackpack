#!/usr/bin/env python3
"""Run the deterministic Topic 30 self-check in eight fresh processes."""
...
def main() -> int:
    ...
    # A private copy prevents a concurrent rebuild from replacing the binary
    # between hashing and process creation.
    ...
    with (output / "processes.jsonl").open("w", encoding="utf-8") as ledger:
        for run in range(1, RUNS + 1):
            ...
            (output / f"run-{run:02d}.stdout").write_bytes(stdout)
            (output / f"run-{run:02d}.stderr").write_bytes(stderr)
            passed = (
                not timed_out
                and exit_code == 0
